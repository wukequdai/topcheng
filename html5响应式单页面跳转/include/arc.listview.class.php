...
class ListView
{
    ...
    function GetPageListST($list_len,$listitem="index,end,pre,next,pageno")
    {
        ...
        if($totalpage<=1 && $this->TotalResult>0)
        {
            ...
            return "<span class=\"disabled prev\">«</span><span class=\"current\">1</span><span class=\"disabled next\">»</span>\r\n";
        }
        ...
        $purl = $this->GetCurUrl();
        $maininfo = "<span class=\"disabled\">共 <strong>{$totalpage}</strong>页<strong>".$this->TotalResult."</strong>条</span>\r\n";
        ...
        if($this->PageNo != 1)
        {
            $prepage.="<a class=\"prev\" href='".str_replace("{page}",$prepagenum,$tnamerule)."'>«</a>\r\n";
            $indexpage="<a href='".str_replace("{page}",1,$tnamerule)."'>首页</a>\r\n";
        }
        else
        {
		    $prepage.="<span class=\"disabled prev\">«</span>\r\n";
        ...
        {
            $nextpage.="<a class=\"next\" href='".str_replace("{page}",$nextpagenum,$tnamerule)."'>»</a>\r\n";
            $endpage="<a href='".str_replace("{page}",$totalpage,$tnamerule)."'>末页</a>\r\n";
        }
        else
        {
		    $nextpage.="<span class=\"disabled next\">»</span>\r\n";
    ...
    function GetPageListDM($list_len,$listitem="index,end,pre,next,pageno")
    {
        ...
        if($this->PageNo != 1)
        {
            $prepage.="<a class=\"prev\" href='".$purl."PageNo=$prepagenum'>«</a>\r\n";
        ...
        if($this->PageNo!=$totalpage && $totalpage>1)
        {
            $nextpage.="<a class=\"next\" href='".$purl."PageNo=$nextpagenum'>»</a>\r\n";
        ...
        for($j;$j<=$total_list;$j++)
        {
            if($j==$this->PageNo)
            {
                $listdd.= "<span class=\"current\"><a>$j</span>\r\n";
            }
            else
            {
