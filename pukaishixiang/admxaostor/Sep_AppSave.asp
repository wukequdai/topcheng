...
Keyword=DelStr(trim(request.form("Keyword")))
...
if ClassID="" then
	...
	if ClassID<=0 then
		...
		set tClass=conn.execute("select ClassName,Depth,ParentPath,Child,ParentID From AppClass where ClassID=" & ClassID)
...
if founderr=false then
	...
	if Action="SaveAdd" then
		sql="select * from App where (id is null)"
...
<link href='Images/hi.xaos.css' rel='stylesheet' />
 ...
 <div class="loading_onclick" style="padding-top:12px; line-height:22px;">
    ...
    【<a href="Sep_AppModify.asp?ID=<%=ID%>&ClassID=<%=ClassID%>">修改信息</a>】&nbsp;
    【<a href="Sep_AppAdd.asp?ClassID=<%=ClassID%>">继续添加</a>】&nbsp;
    【<a href="Sep_App.asp?ClassID=<%=ClassID%>">信息管理</a>】
    </p>
 </div>
 </div>
