...
<table class="nsw_pro_list">
  <tr>
      ...
      <td>
      <select name='ClassID' class="select"><option value=''>请选择所属的栏目</option><%call Admin_ShowClass_Option(3,ClassID)%></select> 
  ...
  <tr>
      ...
      <td> <input name="Keyword" type="text" id="Keyword" size="60" maxlength="50" /></td>
  </tr>
  <%end if%>
  <%if SubEliteA=1 then%>
  <tr>
  ...
  <%end if%>
  <tr>
      <td align="right" valign="top">简述：</td>
  ...
  <tr>
      ...
      <td><input name="OrderID" type="text" id="OrderID" size="5" maxlength="5" onKeyUp="value=value.replace(/[^\d]/g,'')" /> 
  ...
  <tr>
      ...
      <input name="Cancel" type="button" id="Cancel" value="取消" onClick="window.location.href='Sep_App.asp'" class="button" />
	  </td>
  </tr>
</table>
</div>
