...
if len(ClassID)>0 then
	sqlClass="select RootID,ParentID,ParentPath,ClassID,ClassName,Depth From ProductClass where ClassID=" & ClassID
...
<form name="del" method="Post" onSubmit="return CheckForm(this);">
<div class="wel_table">
<table class="nsw_pro_list">
    ...
    <tr id="tabHeader">
        ...
        <td width="*" ><strong><%=Sep_Title%>名称</strong></td>
...
<tr>
    ...
    <%if SubEliteP=1 then%>
		<%if rs("Elite")=1 Then%>
            <a href="Sep_ProductDel.asp?ID=<%=rs("ID")%>&Action=tuino&ClassID=<%=rs("ClassID")%>" onClick="return ConfirmTui();" class="pro_rec">已荐</a>
        <%else%>
            <a href="Sep_ProductDel.asp?ID=<%=rs("ID")%>&Action=tui&ClassID=<%=rs("ClassID")%>" onClick="return ConfirmTui2();" class="pro_rec2">推荐</a>
        <%end if%>
    <%end if%>
	<a href="Sep_ProductModify.asp?ID=<%=rs("ID")%>&ClassID=<%=ClassID%>" class="pro_edit">修改</a>
	<a href="Sep_ProductDel.asp?ID=<%=rs("ID")%>&Action=Del&ClassID=<%=ClassID%>" onClick="return ConfirmDel();" class="pro_del">删除</a></td>
</tr>
<%
Rs.MoveNext
i=i+1
...
<input name="button" type='button' class="button3" value='删除选定的信息' onClick="this.form.action='Sep_ProductDel.asp?Action=Del';this.form.submit();return ConfirmDel();">
<input name="button" type='button' class="button3" value='保存输入的排序' onClick="this.form.action='Sep_ProductDel.asp?Action=Order';this.form.submit();return ConfirmOrder();"></td>
</tr>
</table>
</div>
</form>
