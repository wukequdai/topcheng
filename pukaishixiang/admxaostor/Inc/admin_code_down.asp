...
sub Admin_ShowRootClass(RootID,ClassID)
	dim sqlRoot,rsRoot
	sqlRoot="select ClassID,ClassName,RootID,Child From DownClass where ParentID=0 and RootID="&RootID&" order by RootID"
	...
	if rsRoot.bof and rsRoot.eof then
		...
		sqlSmall="select ClassID,ClassName,ParentID,Depth,NextID From DownClass where Depth>0 and RootID="&rsRoot(2)&" order by RootID,OrderID"
	...
	if ClassID="" then
		...
		if UBound(Sep_ParentPath)>0 then
			For i=1 To UBound(Sep_ParentPath)
				set rscn= conn.execute("select ClassName from DownClass where ClassID="&Sep_ParentPath(i))
