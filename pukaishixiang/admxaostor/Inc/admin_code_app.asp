...
sub Admin_ShowRootClass(RootID,ClassID)
	dim sqlRoot,rsRoot
	sqlRoot="select ClassID,ClassName,RootID,Child From AppClass where ParentID=0 and RootID="&RootID&" and ClassType<2 order by RootID"
	...
	if rsRoot.bof and rsRoot.eof then
		...
		sqlSmall="select ClassID,ClassName,ParentID,Depth,NextID From AppClass where Depth>0 and RootID="&rsRoot(2)&" and ClassType<2 order by RootID,OrderID"
...
sub Admin_ShowClass_Option(ShowType,CurrentID)
	...
	for i=0 to ubound(arrShowLine)
		arrShowLine(i)=False
	next
	sqlClass="Select * From AppClass where Depth=0 and ClassType<2 order by RootID,OrderID"
	...
	if rsClass.bof and rsClass.bof then
		...
		do while not rsClass.eof
			if rsClass("Child")>0 then
				Set rsSmall= Server.CreateObject("ADODB.Recordset")
				sqlSmall="select * From AppClass where Depth>0 and RootID="&rsClass("RootID")&" and ClassType<2 order by RootID,OrderID"
				rsSmall.open sqlSmall,conn,1,1
	...
	if ClassID="" then
		...
		if UBound(Sep_ParentPath)>0 then
			For i=1 To UBound(Sep_ParentPath)
				set rscn= conn.execute("select ClassName from AppClass where ClassID="&Sep_ParentPath(i))
