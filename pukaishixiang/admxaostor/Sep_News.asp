...
if len(ClassID)>0 then
	sqlClass="select RootID,ParentID,ParentPath,ClassID,ClassName,Depth From NewsClass where ClassID=" & ClassID
...
<form name="del" method="Post" action="Sep_NewsDel.asp" onSubmit="return ConfirmDel();">
<div class="wel_table">
<table class="nsw_pro_list">
  <tr>
      <td colspan="5"><%call Admin_ShowRootClass(RootID,ClassID)%>
      <span style="float:right; color:#F00;">排序：数字越大越靠前</span>
	  </td>
  </tr>
  <tr id="tabHeader">
...
if rs.eof and rs.bof then
	response.write "<tr><td align='center' colspan=5>对不起，没有找到相关信息！</td></tr></table>"
else
...
<tr>
    ...
    <input name="OrderID" type="text" value="<%=rs("OrderID")%>" size="5" /></td>
	<td align="center"><%= FormatDateTime(rs("UpdateTime"),2) %></td>
	<td align="center">
    <%if SubEliteN=1 then%>
		<%if rs("Elite")=1 then%>
            <a href="Sep_NewsDel.asp?ID=<%=rs("ID")%>&Action=tuino&ClassID=<%=rs("ClassID")%>" onClick="return ConfirmTui();" class="pro_rec">已荐</a>
        <%else%>
            <a href="Sep_NewsDel.asp?ID=<%=rs("ID")%>&Action=tui&ClassID=<%=rs("ClassID")%>" onClick="return ConfirmTui2();" class="pro_rec2">推荐</a>
        <%end if%>
    <%end if%>
	<a href="Sep_NewsModify.asp?ID=<%=rs("ID")%>&ClassID=<%=ClassID%>" class="pro_edit">修改</a>
	<a href="Sep_NewsDel.asp?ID=<%=rs("ID")%>&Action=Del&ClassID=<%=ClassID%>" onClick="return ConfirmDel();" class="pro_del">删除</a></td>
</tr>
<%
Rs.MoveNext
i=i+1
...
<table class="nsw_pro_list">
<tr>
<td width="50" align="center"><input name="chkAll" type="checkbox" id="chkAll" onClick="CheckAll(this.form)" value="checkbox">
<input name="ClassID" type="hidden" id="ClassID" value="<%=ClassID%>"> 全选</td>
<td>
<input name="button" type='button' class="button3" value='删除选定的信息' onClick="this.form.action='Sep_NewsDel.asp?Action=Del';this.form.submit();return ConfirmDel();">
<input name="button" type='button' class="button4" value='保存输入的排序' onClick="this.form.action='Sep_NewsDel.asp?Action=Order';this.form.submit();return ConfirmOrder();"></td>
</tr>
</table>
</div>
</form>
