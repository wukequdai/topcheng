...
<div class="nsw_bread_tit">
    <span class="nsw_add">当前位置：<%=SubTitleN%>分类 > 栏目管理</span> 
...
sub main()
	...
	sqlClass="select * From NewsClass where Depth=0 order by RootID,OrderID"
...
<div class="bord_gray">
    <div class="graybar_one f_cb">
        <div class="wel_name">
        <a class="easyui-linkbutton" href="Admin_Class_News.asp" iconCls="icon-commonedit" plain="true">栏目管理</a> 
        <a class="easyui-linkbutton" href="Admin_Class_News.asp?Action=Add" iconCls="icon-add" plain="true">添加栏目</a> 
        <%If session(SiteSn & "AdminPurview")=1 Then%>
        <a class="easyui-linkbutton" href="Admin_Class_News.asp?Action=Order" iconCls="icon-order" plain="true">一级栏目排序</a> 
        <a class="easyui-linkbutton" href="Admin_Class_News.asp?Action=OrderN" iconCls="icon-ordern" plain="true">N级栏目排序</a>
        <%end if%>
        </div>
    </div>
</div>
  ...
  <tr id="tabBody">
    ...
    do while not rsClass.eof
	  ...
	  if rsClass("Depth")=0 then
      	if SubChildN=1 then
      		response.Write "<a href='Admin_Class_News.asp?Action=Add&ParentID="&rsClass("ClassID")&"' class='pro_add'>添加子栏目</a>"
		end if
      end if
      response.Write "<a href='Admin_Class_News.asp?Action=Modify&ClassID="&rsClass("ClassID")&"' class='pro_edit'>修改</a>"
	  response.Write "<a href='Admin_Class_News.asp?Action=Del&ClassID="&rsClass("ClassID")&"'"
	  ...
	  if rsClass("Child")>0 then
		...
		sqlSmall="select ClassID,ClassName,ParentID,Depth,NextID,Child,ClassType From NewsClass where Depth>0 and RootID="&rsClass("RootID")&" order by RootID,OrderID"
		...
		'if rsSmall("Depth")=1 then
           'response.Write "<a href='Admin_Class_News.asp?Action=Add&ParentID="&rsSmall("ClassID")&"' class='pro_add'>添加子栏目</a>"
        'end if
        response.Write "<a href='Admin_Class_News.asp?Action=Modify&ClassID="&rsSmall("ClassID")&"' class='pro_edit'>修改</a>"
        response.Write "<a href='Admin_Class_News.asp?Action=Del&ClassID="&rsSmall("ClassID")&"'"
...
sub AddClass()
%>
<form name="myform" method="post" action="Admin_Class_News.asp" onSubmit="return check()">
<div class="wel_table">
<table class="nsw_pro_list">
    <tr id="tabHeader">
    ...
    <tr>
      ...
      <td><input name="ClassType" type="radio" value="0" checked /> 列表 
      <input name="ClassType" type="radio" value="1" /> 图片
      <input name="ClassType" type="radio" value="2" /> 单页  </td>
    </tr>
    <%if SubBannerN=1 then%>
    <tr>
    ...
    <tr>
      ...
      <input name="Cancel" type="button" id="Cancel" value="取消" onClick="window.location.href='Admin_Class_News.asp'" class="button">
      </td>
    </tr>
  </table>
  </div>
...
<table class="nsw_pro_list">
	...
	if rsClass("ParentID")<=0 then
		...
		sqlParentClass="Select * From NewsClass where ClassID in (" & rsClass("ParentPath") & ") order by Depth"
    ...
    <span style="margin-left:100px;">
      如果改变所属栏目，请 <strong><a href='Admin_Class_News.asp?Action=Move&ClassID=<%=ClassID%>'><font color="#ff0000">点此移动栏目</font></a></strong>
    </span></td>
    </tr>
    <tr>
      <td align="center"><strong>栏目名称：</strong></td>
    ...
    <tr>
      <td align="center"><strong>栏目形式：</strong></td>
      <td><input name="ClassType" type="radio" value="0" <%if rsClass("ClassType")=0 then%>checked<%end if%> /> 列表
      <input name="ClassType" type="radio" value="1" <%if rsClass("ClassType")=1 then%>checked<%end if%> /> 图片
      <input name="ClassType" type="radio" value="2" <%if rsClass("ClassType")=2 then%>checked<%end if%> /> 单页 </td>
    </tr>
    <%if SubBannerN=1 then%>
    <tr>
        <td align="center"><strong>栏目图片：</strong></td>
...
%>
<form name="form1" method="post" action="Admin_Class_News.asp">
<div class="wel_table">
<table class="nsw_pro_list">
    <tr id="tabHeader">
    ...
    <tr>
	  ...
	  <input name="Action" type="hidden" id="Action" value="SaveMove">
        ...
        &nbsp;
      <input name="Cancel" type="button" id="Cancel" value="取消" onClick="window.location.href='Admin_Class_News.asp'" class="button"></td></tr>
  </table>
  </div>
</form>
<%
...
<%
	if j>1 then
  		response.write "<form action='Admin_Class_News.asp?Action=UpOrder' method='post'><td width='250' >"
	...
	if iCount>j then
  		response.write "<form action='Admin_Class_News.asp?Action=DownOrder' method='post'><td width='250'>"
...
<%
	if rsClass("ParentID")>0 then   '如果不是一级栏目，则算出相同深度的栏目数目，得到该栏目在相同深度的栏目中所处位置（之上或者之下的栏目数）
		...
		set trs=conn.execute("select count(ClassID) From NewsClass where ParentID="&rsClass("ParentID")&" and OrderID<"&rsClass("OrderID")&"")
		UpMoveNum=trs(0)
		if isnull(UpMoveNum) then UpMoveNum=0
		if UpMoveNum>0 then
  			response.write "<form action='Admin_Class_News.asp?Action=UpOrderN' method='post'><td width='250'>"
			response.write "<select name=MoveNum size=1><option value=0>向上移动</option>"
		...
		if DownMoveNum>0 then
  			response.write "<form action='Admin_Class_News.asp?Action=DownOrderN' method='post'><td width='250'>"
			response.write "<select name=MoveNum size=1><option value=0>向下移动</option>"
...
sub SaveAdd()
	...
	set rs = conn.execute("select Max(ClassID) From NewsClass")
	MaxClassID=rs(0)
	...
	set rs = conn.execute("select Max(RootID) From NewsClass")
	MaxRootID=rs(0)
	...
	if ParentID>0 then
		sql="select * From NewsClass where ClassID=" & ParentID & ""
		...
		if FoundErr=True then
			...
			if Child>0 then
				...
				set rsPrevOrderID=conn.execute("select Max(OrderID) From NewsClass where ParentID=" & ParentID)
				PrevOrderID=rsPrevOrderID(0)
				set trs=conn.execute("select ClassID from NewsClass where ParentID=" & ParentID & " and OrderID=" & PrevOrderID)
				PrevID=trs(0)

				'得到同一父栏目但比本栏目级数大的子栏目的最大OrderID，如果比前一个值大，则改用这个值。
				set rsPrevOrderID=conn.execute("select Max(OrderID) From NewsClass where ParentPath like '" & ParentPath & ",%'")
		...
		if MaxRootID>0 then
			set trs=conn.execute("select ClassID from NewsClass where RootID=" & MaxRootID & " and Depth=0")
			PrevID=trs(0)
			trs.close
		else
			PrevID=0
	...
	if PrevID>0 then
		conn.execute("update NewsClass set NextID=" & ClassID & " where ClassID=" & PrevID)
	end if

	if ParentID>0 then
		'更新其父类的子栏目数
		conn.execute("update NewsClass set child=child+1 where ClassID="&ParentID)

		'更新该栏目排序以及大于本需要和同在本分类下的栏目排序序号
		conn.execute("update NewsClass set OrderID=OrderID+1 where rootid=" & rootid & " and OrderID>" & PrevOrderID)
		conn.execute("update NewsClass set OrderID=" & PrevOrderID & "+1 where ClassID=" & ClassID)
	end if

    call CloseConn()
	call WriteSuccessMsg("添加","Admin_Class_News.asp")
end sub
...
sub SaveModify()
	...
	'if rsClass("Child")>0 then
	'	FoundErr=True
	'	ErrMsg=ErrMsg & "<br><li>本栏目有子栏目，所以不能修改。</li>"
	'end if
    ...
    call CloseConn()
	call WriteSuccessMsg("更新","Admin_Class_News.asp")
end sub
...
sub DeleteClass()
	...
	NextID=rs("NextID")
	if rs("Depth")>0 then
		conn.execute("update NewsClass set child=child-1 where ClassID=" & rs("ParentID"))
	end if
	rs.delete
	rs.update
	rs.close
	...
	if PrevID>0 then
		conn.execute "update NewsClass set NextID=" & NextID & " where ClassID=" & PrevID
	end if
	if NextID>0 then
		conn.execute "update NewsClass set PrevID=" & PrevID & " where ClassID=" & NextID
	end if
	call CloseConn()
	call WriteSuccessMsg("删除","Admin_Class_News.asp")
...
sub ClearClass()
	...
	set rs=conn.execute("select ClassID,Child,ParentPath from NewsClass where ClassID=" & ClassID)
	if rs.bof and rs.eof then
		FoundErr=True
		ErrMsg=ErrMsg & "<br><li>栏目不存在，或者已经被删除</li>"
		exit sub
	...
	if rs(1)>0 then
		set trs=conn.execute("select ClassID from NewsClass where ParentID=" & rs(0))
		do while not trs.eof
			strClassID=strClassID & "," & trs(0)
			trs.movenext
		loop
		trs.close
		set trs=conn.execute("select ClassID from NewsClass where ParentPath like '" & rs(2) & "," & rs(0) & ",%'")
		do while not trs.eof
			strClassID=strClassID & "," & trs(0)
			trs.movenext
		loop
...
sub SaveMove()
	...
	if rsClass("ParentID")<>rParentID then   '更改了所属栏目，则要做一系列检查
		...
		if rsClass("ParentID")=0 then
			if rParentID>0 then
				set trs=conn.execute("select rootid From NewsClass where ClassID="&rParentID)
			...
			set trs=conn.execute("select ClassID From NewsClass where ParentPath like '"&rsClass("ParentPath")&"," & rsClass("ClassID") & "%' and ClassID="&rParentID)
  ...
  dim mrs,MaxRootID
  set mrs=conn.execute("select max(rootid) From NewsClass")
  MaxRootID=mrs(0)
  set mrs=nothing
  if isnull(MaxRootID) then
	MaxRootID=0
  ...
  if clng(parentid)<>rParentID and not (iParentID=0 and rParentID=0) then  '假如更改了所属栏目
	...
	if iParentID>0 and rParentID=0 then  	'如果原来不是一级分类改成一级分类
		...
		'更新当前栏目数据
		conn.execute("update NewsClass set depth=0,OrderID=0,rootid="&maxrootid&",parentid=0,ParentPath='0',PrevID=" & PrevID & ",NextID=0 where ClassID="&ClassID)
		'如果有下属栏目，则更新其下属栏目数据。下属栏目的排序不需考虑，只需更新下属栏目深度和一级排序ID(rootid)数据
		if child>0 then
			i=0
			ParentPath=ParentPath & ","
			set rs=conn.execute("select * From NewsClass where ParentPath like '%"&ParentPath & ClassID&"%'")
			do while not rs.eof
				i=i+1
				mParentPath=replace(rs("ParentPath"),ParentPath,"")
				conn.execute("update NewsClass set depth=depth-"&depth&",rootid="&maxrootid&",ParentPath='"&mParentPath&"' where ClassID="&rs("ClassID"))
				rs.movenext
			loop
			rs.close
			set rs=nothing
		...
		conn.execute("update NewsClass set child=child-1 where ClassID="&iParentID)

	elseif iParentID>0 and rParentID>0 then    '如果是将一个分栏目移动到其他分栏目下
		'得到当前栏目的下属子栏目数
		ParentPath=ParentPath & ","
		set rs=conn.execute("select count(*) From NewsClass where ParentPath like '%"&ParentPath & ClassID&"%'")
		ClassCount=rs(0)
		...
		'获得目标栏目的相关信息
		set trs=conn.execute("select * From NewsClass where ClassID="&rParentID)
		if trs("Child")>0 then
			'得到与本栏目同级的最后一个栏目的OrderID
			set rsPrevOrderID=conn.execute("select Max(OrderID) From NewsClass where ParentID=" & trs("ClassID"))
			PrevOrderID=rsPrevOrderID(0)
			...
			'得到同一父栏目但比本栏目级数大的子栏目的最大OrderID，如果比前一个值大，则改用这个值。
			set rsPrevOrderID=conn.execute("select Max(OrderID) From NewsClass where ParentPath like '" & trs("ParentPath") & "," & trs("ClassID") & ",%'")
			if (not(rsPrevOrderID.bof and rsPrevOrderID.eof)) then
				if not IsNull(rsPrevOrderID(0))  then
			 		if rsPrevOrderID(0)>PrevOrderID then
						PrevOrderID=rsPrevOrderID(0)
		...
		conn.execute("update NewsClass set OrderID=OrderID+" & ClassCount & "+1 where rootid=" & trs("rootid") & " and OrderID>" & PrevOrderID)

		'更新当前栏目数据
		conn.execute("update NewsClass set depth="&trs("depth")&"+1,OrderID="&PrevOrderID&"+1,rootid="&trs("rootid")&",ParentID="&rParentID&",ParentPath='" & trs("ParentPath") & "," & trs("ClassID") & "',PrevID=" & PrevID & ",NextID=0 where ClassID="&ClassID)

		'如果有子栏目则更新子栏目数据，深度为原来的相对深度加上当前所属栏目的深度
		set rs=conn.execute("select * From NewsClass where ParentPath like '%"&ParentPath&ClassID&"%' order by OrderID")
		i=1
		do while not rs.eof
			i=i+1
			iParentPath=trs("ParentPath") & "," & trs("ClassID") & "," & replace(rs("ParentPath"),ParentPath,"")
			conn.execute("update NewsClass set depth=depth-"&depth&"+"&trs("depth")&"+1,OrderID="&PrevOrderID&"+"&i&",rootid="&trs("rootid")&",ParentPath='"&iParentPath&"' where ClassID="&rs("ClassID"))
			rs.movenext
		loop
		rs.close
		set rs=nothing
		...
		'更新所指向的上级栏目的子栏目数
		conn.execute("update NewsClass set child=child+1 where ClassID="&rParentID)

		'更新其原父类的子栏目数
		conn.execute("update NewsClass set child=child-1 where ClassID="&iParentID)
	else    '如果原来是一级栏目改成其他栏目的下属栏目
		'得到移动的栏目总数
		set rs=conn.execute("select count(*) From NewsClass where rootid="&rootid)
		ClassCount=rs(0)
		rs.close
		set rs=nothing
		...
		conn.execute("update NewsClass set PrevID=" & PrevID & ",NextID=0 where ClassID=" & ClassID)
		set rs=conn.execute("select * From NewsClass where rootid="&rootid&" order by OrderID")
		i=0
		do while not rs.eof
			i=i+1
			if rs("parentid")=0 then
				ParentPath=trs("ParentPath") & "," & trs("ClassID")
				conn.execute("update NewsClass set depth=depth+"&trs("depth")&"+1,OrderID="&PrevOrderID&"+"&i&",rootid="&trs("rootid")&",ParentPath='"&ParentPath&"',parentid="&rParentID&" where ClassID="&rs("ClassID"))
			else
				ParentPath=trs("ParentPath") & "," & trs("ClassID") & "," & replace(rs("ParentPath"),"0,","")
				conn.execute("update NewsClass set depth=depth+"&trs("depth")&"+1,OrderID="&PrevOrderID&"+"&i&",rootid="&trs("rootid")&",ParentPath='"&ParentPath&"' where ClassID="&rs("ClassID"))
			end if
			rs.movenext
		loop
		rs.close
  ...
  call CloseConn()
  call WriteSuccessMsg("移动","Admin_Class_News.asp")
end sub
...
sub UpOrder()
	...
	'得到本栏目的PrevID,NextID
	set rs=conn.execute("select PrevID,NextID from NewsClass where ClassID=" & ClassID)
	PrevID=rs(0)
	NextID=rs(1)
	rs.close
	set rs=nothing
	...
	MaxRootID=mrs(0)+1
	'先将当前栏目移至最后，包括子栏目
	conn.execute("update NewsClass set RootID=" & MaxRootID & " where RootID=" & cRootID)
	...
	do while not rsOrder.eof
		...
		conn.execute("update NewsClass set RootID=RootID+1 where RootID=" & tRootID)
		i=i+1
		if i>MoveNum then
			rsOrder("PrevID")=ClassID
			rsOrder.update
			conn.execute("update NewsClass set NextID=" & rsOrder("ClassID") & " where ClassID=" & ClassID)
			exit do
		end if
		rsOrder.movenext
	loop
	rsOrder.movenext
	if rsOrder.eof then
		conn.execute("update NewsClass set PrevID=0 where ClassID=" & ClassID)
	else
		rsOrder("NextID")=ClassID
		rsOrder.update
		conn.execute("update NewsClass set PrevID=" & rsOrder("ClassID") & " where ClassID=" & ClassID)
	end if
	rsOrder.close
	set rsOrder=nothing

	'然后再将当前栏目从最后移到相应位置，包括子栏目
	conn.execute("update NewsClass set RootID=" & tRootID & " where RootID=" & MaxRootID)
	call CloseConn()
	response.Redirect "Admin_Class_News.asp?Action=Order"
end sub
...
sub DownOrder()
	...
	rsOrder.movenext
	if rsOrder.eof then
		conn.execute("update NewsClass set NextID=0 where ClassID=" & ClassID)
	else
		rsOrder("PrevID")=ClassID
		rsOrder.update
		conn.execute("update NewsClass set NextID=" & rsOrder("ClassID") & " where ClassID=" & ClassID)
...
sub UpOrderN()
	...
	'要移动的栏目信息
	set rs=conn.execute("select ParentID,OrderID,ParentPath,child,PrevID,NextID From NewsClass where ClassID="&ClassID)
	ParentID=rs(0)
	OrderID=rs(1)
	...
	set rs=nothing
	if child>0 then
		set rs=conn.execute("select count(*) From NewsClass where ParentPath like '%"&ParentPath&"%'")
		oldorders=rs(0)
		rs.close
		set rs=nothing
	else
	...
	do while not rs.eof
		tOrderID=rs(1)
		conn.execute("update NewsClass set OrderID="&tOrderID+oldorders+i&" where ClassID="&rs(0))
		if rs(2)>0 then
			ii=i+1
			set trs=conn.execute("select ClassID,OrderID From NewsClass where ParentPath like '%"&rs(3)&","&rs(0)&"%' order by OrderID")
			if not (trs.eof and trs.bof) then
				do while not trs.eof
					conn.execute("update NewsClass set OrderID="&tOrderID+oldorders+ii&" where ClassID="&trs(0))
					ii=ii+1
					trs.movenext
				loop
			end if
		...
		if i>MoveNum then
			rs(4)=ClassID
			rs.update
			conn.execute("update NewsClass set NextID=" & rs(0) & " where ClassID=" & ClassID)
			exit do
		end if
		rs.movenext
	loop
	...
	if rs.eof then
		...
		rs(5)=ClassID
		rs.update
		conn.execute("update NewsClass set PrevID=" & rs(0) & " where ClassID=" & ClassID)
	end if
	rs.close
	set rs=nothing

	'更新所要排序的栏目的序号
	conn.execute("update NewsClass set OrderID="&tOrderID&" where ClassID="&ClassID)
	'如果有下属栏目，则更新其下属栏目排序
	if child>0 then
		i=1
		set rs=conn.execute("select ClassID From NewsClass where ParentPath like '%"&ParentPath&"%' order by OrderID")
		do while not rs.eof
			conn.execute("update NewsClass set OrderID="&tOrderID+i&" where ClassID="&rs(0))
			i=i+1
			rs.movenext
		loop
		rs.close
	...
	call CloseConn()
	response.Redirect "Admin_Class_News.asp?Action=OrderN"
end sub
...
sub DownOrderN()
	...
	ii=0     '同级栏目和子栏目
	do while not rs.eof
		conn.execute("update NewsClass set OrderID="&OrderID+ii&" where ClassID="&rs(0))
		if rs(2)>0 then
			set trs=conn.execute("select ClassID,OrderID From NewsClass where ParentPath like '%"&rs(3)&","&rs(0)&"%' order by OrderID")
			if not (trs.eof and trs.bof) then
				do while not trs.eof
					ii=ii+1
					conn.execute("update NewsClass set OrderID="&OrderID+ii&" where ClassID="&trs(0))
					trs.movenext
				loop
			end if
			trs.close
	...
	'更新所要排序的栏目的序号
	conn.execute("update NewsClass set OrderID="&OrderID+ii&" where ClassID="&ClassID)
	'如果有下属栏目，则更新其下属栏目排序
	if child>0 then
		i=1
		set rs=conn.execute("select ClassID From NewsClass where ParentPath like '%"&ParentPath&"%' order by OrderID")
		do while not rs.eof
			conn.execute("update NewsClass set OrderID="&OrderID+ii+i&" where ClassID="&rs(0))
			i=i+1
			rs.movenext
		loop
		rs.close
