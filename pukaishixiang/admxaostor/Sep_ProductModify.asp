...
FoundErr=False
if ID="" then
	response.Redirect("Sep_Product.asp")
end if
...
<table class="nsw_pro_list">
  ...
  <tr>
      <td align="right" valign="top"><%=Sep_Title%>参数：</td>
      <td><textarea name="Canshu" style="width:98%; height:100px;"><%=htmlcode(rs("Canshu"))%></textarea></td>
  </tr>
  <%end if%>
  <tr>
  ...
  <%if SubPicP=1 then%>
      ...
      <tr <%if SubPicP=0 then response.Write "style='display:none;'"%>>
  ...
  <tr>
      ...
      <input name="Cancel" type="button" id="Cancel" value="取消" onClick="window.location.href='Sep_Product.asp?ClassID=<%=ClassID%>'" class="button" />
      </td>
  </tr>
</table>
</div>
